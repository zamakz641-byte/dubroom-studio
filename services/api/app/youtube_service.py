...
def start(url: str, title: str | None = None) -> dict[str, Any]:
    candidate = validate_url(url)
    if not runtime_ready():
...
def run(download_id: str) -> None:
    ...
    _patch_state(download_id, status="downloading", message="Downloading the best video and audio")
    ...
    try:
        ...
        for raw in process.stdout:
            ...
            if line.startswith("__DUB_PROGRESS__"):
                ...
                _patch_state(download_id, path=line.removeprefix("__DUB_FILE__").strip(), progress=98, status="processing", message="Finalizing the video container")
        code = process.wait()
        ...
        if not path.is_file():
            ...
            path = next((item for item in candidates if item.is_file() and item.suffix.lower() in {".mp4", ".mkv", ".webm", ".mov"}), Path())
        if not path.is_file():
            raise RuntimeError("The download finished but no video file was produced")
        _patch_state(download_id, status="completed", progress=100, path=str(path.resolve()), message="Video ready", eta=None, speed=None)
